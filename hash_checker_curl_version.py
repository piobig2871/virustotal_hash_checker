...
def hash_checker_curl_version(hashes: List, api_key: str):
    ret = {"hashes": []}
    for hash in hashes:
        response = os.system(
            f"curl --request GET --url  https://www.virustotal.com/api/v3/files/{hash} --header 'x-apikey: {api_key}'"
        )
...
print(hash_checker_curl_version(hashes=hashes, api_key=api_key))
